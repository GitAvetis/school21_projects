...
START_TEST(test_s21_strncat_from_file) {
  ...
  while (fgets(buffer, MAX_LEN_STR, fp) != NULL) {
    ...
    for (s21_size_t i = 0; i < 100; i += 10) {
      ...
      ck_assert_str_eq(s21_strncat(dest_s21, buffer, i),
                       strncat(dest, buffer, i));
    }
  }
  fclose(fp);
}
...
Suite *s21_strncat_test(void) {
  ...
  tcase_add_test(tc_core, test_s21_strncat_from_file);
